 ...
 * @version $Id: JRDesignMeterPlot.java,v 1.1 2008/09/29 16:21:32 guehene Exp $
 */
public class JRDesignMeterPlot extends JRBaseMeterPlot
{
	...
	public void setShape(byte shape) throws JRException
	{
		...
		byte old = this.shape;
		this.shape = shape;
		getEventSupport().firePropertyChange(PROPERTY_SHAPE, (float) old, (float) this.shape);
	}
	...
	public void addInterval(JRMeterInterval interval)
	{
		...
		getEventSupport().fireCollectionElementAddedEvent(PROPERTY_INTERVALS, interval, this.intervals.size() - 1);
	}
	...
	public void setMeterAngle(int meterAngle)
	{
		...
		getEventSupport().firePropertyChange(PROPERTY_METER_ANGLE, (float) old, (float) this.meterAngle);
	}
