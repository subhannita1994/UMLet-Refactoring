 ...
 * @version $Id: JdkGlyphFixTextMeasurer.java,v 1.1 2008/09/29 16:20:17 guehene Exp $
...
public class JdkGlyphFixTextMeasurer extends TextMeasurer
{
	...
	public JdkGlyphFixTextMeasurer(JRCommonText textElement)
	{
		...
		this.attempts = JRProperties.getIntegerProperty(PROPERTY_ATTEMPTS, DEFAULT_ATTEMPTS);
		this.sleep = JRProperties.getIntegerProperty(PROPERTY_ATTEMPT_SLEEP, DEFAULT_ATTEMPT_SLEEP);
		this.catchEmptyStacktrace = JRProperties.getBooleanProperty(PROPERTY_CATCH_EMPTY_STACKTRACE);
	}
		...
		if (stackTrace.length == 0)
		{
			if (log.isDebugEnabled())
			{
				log.debug("Caught exception with no stacktrace; " 
						+ (this.catchEmptyStacktrace ? "" : "not ") + "treating as JDK Glyph exception");
			}
