 ...
 * @version $Id: JRSwapFile.java,v 1.1 2008/09/29 16:20:17 guehene Exp $
 */
public class JRSwapFile
{
	...
	protected synchronized long[] reserveFreeBlocks(int blockCount) throws IOException
	{
		...
		if (growCount > 0)
		{
			...
			long newLength = length + growCount * this.blockSize;
			if (log.isDebugEnabled())
			{
			...
			{
				this.freeBlocks.addLast(length + i * this.blockSize);
			}
		}
