 ...
 * @version $Id: JRJdtCompiler.java,v 1.1 2008/09/29 16:20:08 guehene Exp $
 */
public class JRJdtCompiler extends JRAbstractJavaCompiler {
	...
	public JRJdtCompiler() {
		...
		try //FIXME remove support for pre 3.1 jdt
		{
			...
			this.constrNameEnvAnsBin2Args =
				NameEnvironmentAnswer.class.getConstructor(new Class[] {
						IBinaryType.class, classAccessRestriction });
			this.constrNameEnvAnsCompUnit2Args =
				NameEnvironmentAnswer.class.getConstructor(new Class[] {
		...
		if (!success) {
			try {
				this.constrNameEnvAnsBin =
					NameEnvironmentAnswer.class
						.getConstructor(new Class[] { IBinaryType.class });
				this.constrNameEnvAnsCompUnit =
					NameEnvironmentAnswer.class
						.getConstructor(new Class[] { ICompilationUnit.class });
	...
	protected String compileUnits(
		...
		final ICompilerRequestor requestor =
			getCompilerRequestor(units, problemBuffer);

		ICompilationUnit[] compilationUnits =
			new ICompilationUnit[units.length];
		for (int i = 0; i < compilationUnits.length; i++) {
			compilationUnits[i] =
				new CompilationUnit(
					units[i].getSourceCode(),
					units[i].getName());
		}
				...
				try {
					...
					if (classIdx >= 0) {
						ICompilationUnit compilationUnit =
							new CompilationUnit(
								units[classIdx].getSourceCode(),
								className);
						if (JRJdtCompiler.this.is2ArgsConstr) {
							return (NameEnvironmentAnswer) JRJdtCompiler.this.constrNameEnvAnsCompUnit2Args
								.newInstance(new Object[] { compilationUnit,
										null });
						}

						return (NameEnvironmentAnswer) JRJdtCompiler.this.constrNameEnvAnsCompUnit
							.newInstance(new Object[] { compilationUnit });
					}
					...
					if (is != null) {
						...
						if (JRJdtCompiler.this.is2ArgsConstr) {
							return (NameEnvironmentAnswer) JRJdtCompiler.this.constrNameEnvAnsBin2Args
								.newInstance(new Object[] { classFileReader,
										null });
						}

						return (NameEnvironmentAnswer) JRJdtCompiler.this.constrNameEnvAnsBin
							.newInstance(new Object[] { classFileReader });
					}
				}
					...
					throw new JRRuntimeException(
						"Not able to create NameEnvironmentAnswer",
						e);
				}
	...
	protected ICompilerRequestor getCompilerRequestor(
		final JRCompilationUnit[] units,
		final StringBuffer problemBuffer) {
		final ICompilerRequestor requestor = new ICompilerRequestor() {
			public void acceptResult(CompilationResult result) {
				String className =
					((CompilationUnit) result.getCompilationUnit()).className;
					...
					for (int i = 0; i < problems.length; i++) {
						...
						//if (problem.isError()) 
						{
							...
							if (problem.getSourceStart() >= 0
								...
								problemBuffer.append("\n");
								problemBuffer.append(sourceCode.substring(
									problemStartIndex,
									problemEndIndex));
								problemBuffer.append("\n");
								for (int j = problemStartIndex; j < problem
									.getSourceStart(); j++) {
									problemBuffer.append(" ");
								}
								if (problem.getSourceStart() == problem
									.getSourceEnd()) {
									problemBuffer.append("^");
								}
								else {
									problemBuffer.append("<");
									for (int j = problem.getSourceStart() + 1; j < problem
										.getSourceEnd(); j++) {
										problemBuffer.append("-");
									}
									problemBuffer.append(">");
				...
				if (problemBuffer.length() == 0) {
					...
					for (int i = 0; i < resultClassFiles.length; i++) {
						units[classIdx].setCompileData(resultClassFiles[i]
							.getBytes());
					}
				}
			}
	...
	protected Map getJdtSettings() {
		...
		for (Iterator it = properties.iterator(); it.hasNext();) {
			JRProperties.PropertySuffix property =
	...
	protected void checkLanguage(String language) throws JRException {
		if (!JRReport.LANGUAGE_JAVA.equals(language)) {
			throw new JRException("Language \"" + language
	...
	protected IProblem[] getJavaCompilationErrors(CompilationResult result) {
		try {
			Method getErrorsMethod =
				result.getClass().getMethod("getErrors", null);
