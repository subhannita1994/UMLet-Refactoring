 ...
 * @version $Id: JRDesignElement.java,v 1.1 2008/09/29 16:20:08 guehene Exp $
 */
public abstract class JRDesignElement extends JRBaseElement
{
	...
	protected JRDesignElement(JRDefaultStyleProvider defaultStyleProvider)
	{
		...
		this.positionType = JRElement.POSITION_TYPE_FIX_RELATIVE_TO_TOP;
	}
	...
	public void setY(int y)
	{
		int old = this.y;
		this.y = y;
		getEventSupport().firePropertyChange(PROPERTY_Y, (float) old, (float) this.y);
	}
	...
	public void setHeight(int height)
	{
		int old = this.height;
		this.height = height;
		getEventSupport().firePropertyChange(PROPERTY_HEIGHT, (float) old, (float) this.height);
	}
	...
	public void addPropertyExpression(JRPropertyExpression propertyExpression)
	{
		...
		getEventSupport().fireCollectionElementAddedEvent(PROPERTY_PROPERTY_EXPRESSIONS, 
				propertyExpression, this.propertyExpressions.size() - 1);
	}
	...
	public JRPropertyExpression[] getPropertyExpressions()
	{
		...
		{
			props = (JRPropertyExpression[]) this.propertyExpressions.toArray(
					new JRPropertyExpression[this.propertyExpressions.size()]);
		}
		return props;
	}
