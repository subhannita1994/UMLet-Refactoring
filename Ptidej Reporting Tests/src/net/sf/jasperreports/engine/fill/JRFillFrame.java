 ...
 * @version $Id: JRFillFrame.java,v 1.1 2008/09/29 16:20:01 guehene Exp $
 */
public class JRFillFrame extends JRFillElement implements JRFrame
{
	...
	protected boolean prepare(int availableStretchHeight, boolean isOverflow) throws JRException
	{
		...
		if (!this.filling && !isPrintRepeatedValues() && isValueRepeating() &&
	...
	protected JRTemplateFrame getTemplate()
	{
		...
		{
			if (this.fillBottomBorder) //remove the top border
			{
		...
		if (boxTemplate == null)
		{
			...
			if (this.first)
			{
				if (!this.fillBottomBorder) //remove the bottom border
				{				
