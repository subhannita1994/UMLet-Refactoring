 ...
 * @version $Id: JRTextExporter.java,v 1.1 2008/09/29 16:20:46 guehene Exp $
 */
public class JRTextExporter extends JRAbstractExporter
{
	...
	public void exportReport() throws JRException
	{
		...
		Integer characterWidthParam = (Integer) this.parameters.get(JRTextExporterParameter.CHARACTER_WIDTH);
		if (characterWidthParam != null) {
			this.characterWidth = characterWidthParam.intValue();
			...
			if (pageWidthParam != null) {
				this.pageWidth = pageWidthParam.intValue();
				if (this.pageWidth <= 0)
		...
		Integer characterHeightParam = (Integer) this.parameters.get(JRTextExporterParameter.CHARACTER_HEIGHT);
		if (characterHeightParam != null) {
			this.characterHeight = characterHeightParam.intValue();
			...
			if (pageHeightParam != null) {
				this.pageHeight = pageHeightParam.intValue();
	...
	protected void exportPage(JRPrintPage page) throws IOException
	{
		...
		for (int i = 0; i < this.pageHeight; i++) {
			this.pageData[i] = new char[this.pageWidth];
	...
	protected void exportText(JRPrintText element)
	{
		...
		for (int i = 0; i < rowIndex; i++) {
			...
			System.arraycopy(chars, 0, this.pageData[y + yOffset + i], x + xOffset, chars.length);
		}
	}
	...
	{
		return (int) (((long) this.pageHeight * y) / this.jasperPrint.getPageHeight());
	}
