 ...
 * @version $Id: DrawVisitor.java,v 1.1 2008/09/29 16:22:02 guehene Exp $
 */
public class DrawVisitor implements JRVisitor
{
	...
	public void visitChart(JRChart chart)
	{
		try
		{
			this.imageDrawer.draw(
				this.grx,
				this.convertVisitor.getVisitPrintElement(chart), 
				-chart.getX(), 
				-chart.getY()
				);
		}
	...
	public void visitCrosstab(JRCrosstab crosstab)
	{
		try
		{
			this.frameDrawer.draw(
				this.grx,
				this.convertVisitor.getVisitPrintElement(crosstab), 
				-crosstab.getX(), 
				-crosstab.getY()
				);
		}
	...
	{
		this.ellipseDrawer.draw(
			this.grx,
			this.convertVisitor.getVisitPrintElement(ellipse), 
			-ellipse.getX(), 
			-ellipse.getY()
			);
	}
	...
	public void visitFrame(JRFrame frame)
	{
		try
		{
			this.frameDrawer.draw(
				this.grx,
				this.convertVisitor.getVisitPrintElement(frame), 
				-frame.getX(), 
				-frame.getY()
				);
		}
	...
	public void visitImage(JRImage image)
	{
		try
		{
			this.imageDrawer.draw(
					this.grx,
					this.convertVisitor.getVisitPrintElement(image), 
					-image.getX(), 
					-image.getY()
					);
		}
	...
	public void visitLine(JRLine line)
	{
		this.lineDrawer.draw(
			this.grx,
			this.convertVisitor.getVisitPrintElement(line), 
			-line.getX(), 
			-line.getY()
			);
	}
	...
	public void visitRectangle(JRRectangle rectangle)
	{
		this.rectangleDrawer.draw(
			this.grx,
			this.convertVisitor.getVisitPrintElement(rectangle), 
			-rectangle.getX(), 
			-rectangle.getY()
			);
	}
	...
	public void visitStaticText(JRStaticText staticText)
	{
		this.textDrawer.draw(
			this.grx,
			this.convertVisitor.getVisitPrintElement(staticText), 
			-staticText.getX(), 
			-staticText.getY()
			);
	}
	...
	public void visitSubreport(JRSubreport subreport)
	{
		try
		{
			this.imageDrawer.draw(
				this.grx,
				this.convertVisitor.getVisitPrintElement(subreport), 
				-subreport.getX(), 
				-subreport.getY()
				);
		}
	...
	public void visitTextField(JRTextField textField)
	{
		this.textDrawer.draw(
			this.grx,
			this.convertVisitor.getVisitPrintElement(textField), 
			-textField.getX(), 
			-textField.getY()
			);
	}
