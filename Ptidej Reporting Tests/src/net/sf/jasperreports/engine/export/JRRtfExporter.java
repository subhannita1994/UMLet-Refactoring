 ...
 * @version $Id: JRRtfExporter.java,v 1.1 2008/09/29 16:20:46 guehene Exp $
 */
public class JRRtfExporter extends JRAbstractExporter
{
	...
	protected void exportReportToStream() throws JRException, IOException {
		...
		for(this.reportIndex = 0; this.reportIndex < this.jasperPrintList.size(); this.reportIndex++ ){
			...
			if (pages != null && pages.size() > 0){
				...
				this.writer.write(String.valueOf(twip(this.jasperPrint.getPageWidth())));
				this.writer.write("\\paperh");
				this.writer.write(String.valueOf(twip(this.jasperPrint.getPageHeight())));

				this.writer.write("\\marglsxn0\\margrsxn0\\margtsxn0\\margbsxn0");

				if (this.jasperPrint.getOrientation() == JRReport.ORIENTATION_LANDSCAPE) {
				...
				for (int pageIndex = this.startPageIndex; pageIndex <= this.endPageIndex; pageIndex++) {
					...
					writeAnchor(JR_PAGE_ANCHOR_PREFIX + this.reportIndex + "_" + (pageIndex + 1));
	...
	protected void createColorAndFontEntries() throws JRException {
		for(this.reportIndex = 0; this.reportIndex < this.jasperPrintList.size(); this.reportIndex++ ){
			...
			getFontIndex(new JRBasePrintText(this.jasperPrint.getDefaultStyleProvider()));
	...
	private int getColorIndex(Color color)
	{
		...
		if (colorNdx < 0)
		{
			...
			this.colorBuffer.append("\\red").append(color.getRed())
	...
	private void startElement(JRPrintElement element) throws IOException 
	{
		...
		this.writer.write(String.valueOf(twip(element.getX() + getOffsetX())));
		this.writer.write("\\shpright");
		this.writer.write(String.valueOf(twip(element.getX() + getOffsetX() + element.getWidth())));
		this.writer.write("\\shptop");
		this.writer.write(String.valueOf(twip(element.getY() + getOffsetY())));
		this.writer.write("\\shpbottom");
		this.writer.write(String.valueOf(twip(element.getY() + getOffsetY() + element.getHeight())));
		...
		if (element.getMode() == JRElement.MODE_OPAQUE)
		{
			...
			this.writer.write(String.valueOf(getColorRGB(bgcolor)));
			this.writer.write("}}");
		}
		else
		{
	...
	private void exportPen(JRPen pen) throws IOException 
	{
		...
		this.writer.write(String.valueOf(getColorRGB(pen.getLineColor())));
		this.writer.write("}}");
		...
		this.writer.write(String.valueOf(emu(lineWidth)));
		this.writer.write("}}");
	}
	...
	protected void exportLine(JRPrintLine line) throws IOException 
	{
		...
		this.writer.write(String.valueOf(this.zorder++));
		this.writer.write("\\shpleft");
		this.writer.write(String.valueOf(twip(x)));
		this.writer.write("\\shpright");
		this.writer.write(String.valueOf(twip(x + width)));
		this.writer.write("\\shptop");
		this.writer.write(String.valueOf(twip(y)));
		this.writer.write("\\shpbottom");
		this.writer.write(String.valueOf(twip(y + height)));
	...
	private void exportBorder(JRPen pen, float x, float y, float width, float height) throws IOException 
	{
		...
		this.writer.write(String.valueOf(this.zorder++));
		this.writer.write("\\shpleft");
		this.writer.write(String.valueOf(twip(x)));//FIXMEBORDER starting point of borders seem to have CAP_SQUARE-like appearence at least for Thin
		this.writer.write("\\shpright");
		this.writer.write(String.valueOf(twip(x + width)));
		this.writer.write("\\shptop");
		this.writer.write(String.valueOf(twip(y)));
	...
	protected void exportText(JRPrintText text) throws IOException, JRException {
		...
		this.writer.write(String.valueOf(emu(topPadding)));
		this.writer.write("}}");
		this.writer.write("{\\sp{\\sn dxTextLeft}{\\sv ");
		this.writer.write(String.valueOf(emu(leftPadding)));
		this.writer.write("}}");
		this.writer.write("{\\sp{\\sn dyTextBottom}{\\sv ");
		this.writer.write(String.valueOf(emu(bottomPadding)));
		this.writer.write("}}");
		this.writer.write("{\\sp{\\sn dxTextRight}{\\sv ");
		this.writer.write(String.valueOf(emu(rightPadding)));
		this.writer.write("}}");
		this.writer.write("{\\sp{\\sn fLine}{\\sv 0}}");
...
//		writer.write(String.valueOf(getColorIndex(text.getForecolor())));
		...
		this.writer.write(String.valueOf(getColorIndex(text.getBackcolor())));
		this.writer.write(" ");
...
//		writer.write(String.valueOf(font.getFontSize() * 2));
		...
		this.writer.write(String.valueOf(twip(text.getLineSpacingFactor() * font.getFontSize())));
		this.writer.write(" ");
			...
			)
		{
			...
			this.writer.write("\\f");
			this.writer.write(String.valueOf(getFontIndex(styleFont)));
			...
			if(!(null == styleBackground || styleBackground.equals(text.getBackcolor()))){
				...
				this.writer.write(String.valueOf(getColorIndex(styleBackground)));
			}
			this.writer.write("\\cf");
			this.writer.write(String.valueOf(getColorIndex(styleForeground)));
			this.writer.write(" ");
...
//		}
	...
	protected void exportImage(JRPrintImage printImage) throws JRException, IOException
	{
		...
		if (renderer != null)
		{
			...
			this.writer.write(String.valueOf(this.zorder++));
			this.writer.write("\\shpleft");
			this.writer.write(String.valueOf(twip(printImage.getX() + leftPadding + xoffset + getOffsetX())));
			this.writer.write("\\shpright");
			this.writer.write(String.valueOf(twip(printImage.getX() + leftPadding + xoffset + getOffsetX() + imageWidth)));
			this.writer.write("\\shptop");
			this.writer.write(String.valueOf(twip(printImage.getY() + topPadding + yoffset + getOffsetY())));
			this.writer.write("\\shpbottom");
			this.writer.write(String.valueOf(twip(printImage.getY() + topPadding + yoffset + getOffsetY() + imageHeight)));
		...
		if (customHandler == null)
		{
			switch(link.getHyperlinkType())
			{
				...
				case JRHyperlink.HYPERLINK_TYPE_LOCAL_PAGE :
				{
					...
					{
						hlloc = JR_PAGE_ANCHOR_PREFIX + this.reportIndex + "_" + link.getHyperlinkPage().toString();
						hlfr = hlloc;
					}
					break;
				}
