 ...
 * @version $Id: JRJdbcQueryExecuter.java,v 1.1 2008/09/29 16:20:33 guehene Exp $
 */
public class JRJdbcQueryExecuter extends JRAbstractQueryExecuter
{
	...
	protected void createStatement() throws JRException
	{
		...
		if (this.connection != null && queryString != null && queryString.trim().length() > 0)
		{
			try
			{
				...
				int fetchSize = JRProperties.getIntegerProperty(this.dataset,
	...
	protected void setStatementParameter(int parameterIndex, Class parameterType, Object parameterValue) throws SQLException
	{
		if (java.lang.Boolean.class.isAssignableFrom(parameterType))
		{
			...
			{
				this.statement.setNull(parameterIndex, Types.BIT);
			}
			else
			{
				this.statement.setBoolean(parameterIndex, ((Boolean)parameterValue).booleanValue());
			}
		}
		else if (java.lang.Byte.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.TINYINT);
			}
			else
			{
				this.statement.setByte(parameterIndex, ((Byte)parameterValue).byteValue());
			}
		}
		else if (java.lang.Double.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.DOUBLE);
			}
			else
			{
				this.statement.setDouble(parameterIndex, ((Double)parameterValue).doubleValue());
			}
		}
		else if (java.lang.Float.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.FLOAT);
			}
			else
			{
		...
		else if (java.lang.Integer.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.INTEGER);
			}
			else
			{
				this.statement.setInt(parameterIndex, ((Integer)parameterValue).intValue());
			}
		}
		else if (java.lang.Long.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.BIGINT);
			}
			else
			{
				this.statement.setLong(parameterIndex, ((Long)parameterValue).longValue());
			}
		}
		else if (java.lang.Short.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.SMALLINT);
			}
			else
			{
				this.statement.setShort(parameterIndex, ((Short)parameterValue).shortValue());
			}
		}
		else if (java.math.BigDecimal.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.DECIMAL);
			}
			else
			{
		...
		else if (java.lang.String.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.VARCHAR);
			}
			else
			{
		...
		else if (java.sql.Timestamp.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.TIMESTAMP);
			}
			else
			{
				this.statement.setTimestamp( parameterIndex, (java.sql.Timestamp)parameterValue );
			}
		}
		else if (java.sql.Time.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.TIME);
			}
			else
			{
				this.statement.setTime( parameterIndex, (java.sql.Time)parameterValue );
			}
		}
		else if (java.util.Date.class.isAssignableFrom(parameterType))
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.DATE);
			}
			else
			{
				this.statement.setDate( parameterIndex, new java.sql.Date( ((java.util.Date)parameterValue).getTime() ) );
			}
		}
		else
		{
			if (parameterValue == null)
			{
				this.statement.setNull(parameterIndex, Types.JAVA_OBJECT);
			}
			else
			{
