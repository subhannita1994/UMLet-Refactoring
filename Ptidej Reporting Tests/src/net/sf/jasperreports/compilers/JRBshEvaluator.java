 ...
 * @version $Id: JRBshEvaluator.java,v 1.1 2008/09/29 16:22:21 guehene Exp $
 */
public class JRBshEvaluator extends JREvaluator
{
	...
	public JRBshEvaluator(String bshScript) throws JRException
	{
		...
		try
		{
			this.interpreter.eval(new StringReader(bshScript));
		}
		catch(EvalError e)
		{
		...
		) throws JRException
	{
		try
		{
			...
			this.interpreter.set("fldsm", fldsm);
			this.interpreter.set("varsm", varsm);
			this.interpreter.set("parsm", pars);
			this.interpreter.eval("bshEvaluator = createBshEvaluator()");
