 ...
 * @version $Id: JRAntCompileTask.java,v 1.1 2008/09/29 16:22:22 guehene Exp $
 */
public class JRAntCompileTask extends MatchingTask
{
	...
	public void execute() throws BuildException
	{
		...
		try
		{
			...
			{
				JRProperties.setProperty(JRProperties.COMPILER_TEMP_DIR, String.valueOf(this.tempdir));
			}

			JRProperties.setProperty(JRProperties.COMPILER_KEEP_JAVA_FILE, this.keepjava);

			if (this.compiler != null)
			{
				JRProperties.setProperty(JRProperties.COMPILER_CLASS, this.compiler);
			}

			JRProperties.setProperty(JRProperties.COMPILER_XML_VALIDATION, this.xmlvalidation);

			AntClassLoader classLoader = null;
			if (this.classpath != null)
			{
				JRProperties.setProperty(JRProperties.COMPILER_CLASSPATH, String.valueOf(this.classpath));
				
				ClassLoader parentClassLoader = getClass().getClassLoader();
				classLoader = new AntClassLoader(parentClassLoader, getProject(), this.classpath, true);
	...
	protected void scanSrc() throws BuildException
	{
		...
		for (int i = 0; i < list.length; i++) 
		{
			...
			else
			{
				...
				scanDir(srcdir.getParentFile(), this.destdir != null ? this.destdir : srcdir.getParentFile(), files);
			}
		}
	}
