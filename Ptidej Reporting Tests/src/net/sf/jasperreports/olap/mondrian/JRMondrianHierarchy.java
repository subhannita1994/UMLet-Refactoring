 ...
 * @version $Id: JRMondrianHierarchy.java,v 1.1 2008/09/29 16:20:50 guehene Exp $
 */
public class JRMondrianHierarchy implements JROlapHierarchy
{
	...
	public JRMondrianHierarchy(Hierarchy hierarchy)
	{
		...
		else
		{
			...
			this.levels = new JRMondrianLevel[hierarchyLevels.length];
			for (int i = 0; i < hierarchyLevels.length; i++)
			{
				this.levels[i] = new JRMondrianLevel(hierarchyLevels[i]);
			}
		}
	}
	...
	{
		return this.hierarchy == null ? null : this.hierarchy.getDimension().getName();
	}
