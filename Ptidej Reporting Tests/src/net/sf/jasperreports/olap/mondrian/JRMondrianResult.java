 ...
 * @version $Id: JRMondrianResult.java,v 1.1 2008/09/29 16:20:50 guehene Exp $
 */
public class JRMondrianResult implements JROlapResult
{
	...
	public JRMondrianResult(Result result)
	{
		...
		this.axes = new JRMondrianAxis[resultAxes.length];
		for (int i = 0; i < resultAxes.length; i++)
		{
			AxisOrdinal ordinal = AxisOrdinal.forLogicalOrdinal(i);
			this.axes[i] = new JRMondrianAxis(resultAxes[i], query.getMdxHierarchiesOnAxis(ordinal), factory);
		}
	}
	...
	{
		Cell dataCell = this.result.getCell(axisPositions);
