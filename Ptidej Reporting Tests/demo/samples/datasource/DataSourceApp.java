 ...
 * @version $Id: DataSourceApp.java,v 1.1 2008/09/29 16:21:53 guehene Exp $
 */
public class DataSourceApp {
	...
	public static void main(String[] args) {
		//	args =
		//		new String[] { "fill1", "demo/samples/datasource/DataSourceReport.jasper" };
		args =
			new String[] { "html",
					"demo/samples/datasource/DataSourceReport.jrprint" };
		...
		try {
			...
			if (TASK_FILL_1.equals(taskName)) {
				...
				JasperFillManager.fillReportToFile(
					fileName,
					parameters,
					new JRTableModelDataSource(new CustomTableModel()));
				System.err.println("Filling time : "
						+ (System.currentTimeMillis() - start));
			}
				...
				JasperFillManager
					.fillReportToFile(
						fileName,
						parameters,
						new JRBeanArrayDataSource(CustomBeanFactory
							.getBeanArray()));
				System.err.println("Filling time : "
						+ (System.currentTimeMillis() - start));
			}
				...
				JasperFillManager.fillReportToFile(
					fileName,
					parameters,
					new JRBeanCollectionDataSource(CustomBeanFactory
						.getBeanCollection()));
				System.err.println("Filling time : "
						+ (System.currentTimeMillis() - start));
			}
				...
				File destFile =
					new File(sourceFile.getParent(), jasperPrint.getName()
							+ ".rtf");
	...
	private static void usage() {
		...
		System.out
			.println("\tTasks : fill1 | fill2 | fill3 | fill4 | print | pdf | xml | xmlEmbed | html | rtf | xls | jxl | csv | odt | run");
	}
