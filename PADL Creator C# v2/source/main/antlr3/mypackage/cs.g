...
namespace_member_declaration:
	...
	| attributes?   modifiers?   type_declaration
	 ->^(MEMBER_DECL attributes?   modifiers?   type_declaration)
	;
type_declaration:
	('partial') => 'partial'   (class_declaration ->^(CLASS class_declaration)
				  | struct_declaration
...
class_member_declaration:
	...
	( 'const'   type   constant_declarators   ';' -> ^(CLASSMEMBER 'const' attributes? modifiers? type constant_declarators )
	...
	-> ^(INTERFACE interface_declaration)
	| 'void'   method_declaration -> ^(CLASSMETHOD attributes? modifiers? 'void' method_declaration)
	| type (     (member_name   '(') => method_declaration -> ^(CLASSMETHOD attributes? modifiers? type method_declaration)
		   | (member_name   '{') => property_declaration -> ^(CLASSMETHOD attributes? modifiers? type property_declaration)
		   | (member_name   '.'   'this') => type_name '.' indexer_declaration 
		   | indexer_declaration	//this
	       | field_declaration      -> ^(CLASSMEMBER attributes? modifiers? type field_declaration)// qid
	...
	| constructor_declaration -> ^( CONSTRUCTOR constructor_declaration )	//	| static_constructor_declaration
...
where_clause:
	'where'   booleanean_expression ;
booleanean_expression:
	expression;
...
class_declaration:
	...
	-> type_or_generic   class_base?   type_parameter_constraints_clauses?   class_body
	;
class_base:
...
///////////////////////////////////////////////////////
method_declaration:
	method_header   method_body -> ^(METHOD method_header method_body);
method_header:
...
interface_declaration:
	...
	-> identifier variant_generic_parameter_list? interface_base?   type_parameter_constraints_clauses?   interface_body
	;
interface_modifiers: 
	modifier+ ;
interface_base: 
...
interface_method_declaration:
	identifier   generic_argument_list? '('   formal_parameter_list?   ')'   type_parameter_constraints_clauses?   ';'  -> ^(INTERFACEMETHOD identifier   generic_argument_list? '('   formal_parameter_list?   ')'   type_parameter_constraints_clauses? );
interface_event_declaration: 
	//attributes?   'new'?   
...
if_statement:
	...
	'if'   '('   booleanean_expression   ')'   embedded_statement (('else') => else_statement)?
	;
else_statement:
...
iteration_statement:
	...
	| foreach_statement ;
while_statement:
	'while'   '('   booleanean_expression   ')'   embedded_statement ;
do_statement:
	'do'   embedded_statement   'while'   '('   booleanean_expression   ')'   ';' ;
for_statement:
...
predefined_type:
	  'boolean' | 'byte'   | 'char'   | 'decimal' | 'double' | 'float'  | 'int'    | 'long'   | 'object' | 'sbyte'
