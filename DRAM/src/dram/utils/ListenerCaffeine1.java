 ...
 * @author rachedsa
 */
public class ListenerCaffeine1 implements EngineListener {
		...
		try {
			this.fos =
				new FileOutputStream(
					new File(
						"C:\\Documents and Settings\\rachedsa\\Bureau\\javadraw20.txt"));
			this.osw = new OutputStreamWriter(this.fos);

			this.fosall =
				new FileOutputStream(
					new File(
						"C:\\Documents and Settings\\rachedsa\\Bureau\\javadrawall.txt"));
		...
		try {
			...
			if (getType.equals("methodExit0")) {
				...
				if (this.vectStack.size() == 0) {
					...
					while (found) {
						//						vv = (Vector) vectStack.elementAt(vectStack.size() -
						// 1);
						//						pop(vectStack);
